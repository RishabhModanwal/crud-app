...
app.post("/add", encoder, async (req, res) => {
    try {
        ...
        var errorMessage = {
            email: error.keyValue?.email ? "Email address must be unique" : error.errors?.email ? error.errors.email.message : "",
            phone: error.keyValue?.phone ? "Phone number must be unique" : error.errors?.phone ? error.errors.phone.message : "",
